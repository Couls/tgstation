...
/mob/living/simple_animal/hostile/syndicate/mecha_pilot/proc/exit_mecha(obj/vehicle/sealed/mecha/M)
	...
	for(var/obj/vehicle/sealed/mecha/combat/C in range((vision_range / PIXEL_TILE_SIZE),src))
		if(is_valid_mecha(C))
			target = C
...
/mob/living/simple_animal/hostile/syndicate/mecha_pilot/handle_automated_action()
	...
	if(!.)
		return
	if(!mecha)
		for(var/obj/vehicle/sealed/mecha/combat/mecha_in_range in range(src,vision_range / PIXEL_TILE_SIZE))
